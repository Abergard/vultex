...
[[nodiscard]] auto findQueueFamilies(VkPhysicalDevice device) -> std::optional<std::uint32_t>
{
    ...
    {
        graphicsFamily = std::distance(queueFamilies.begin(), queueIt);
    }

    return graphicsFamily;
}

[[nodiscard]] auto rateDeviceSuitability(const auto& device) -> std::pair<std::int32_t, std::uint32_t>
{
    ...
    // Maximum possible size kf textures affects graphics quality
    score += static_cast<int>(deviceProperties.limits.maxImageDimension2D);

    log::info("Device GPU {} got score: {}", deviceProperties.deviceName, score);

    return {score, queueFamilyIndices.value()};
}
...
[[nodiscard]] auto pickPhysicalDevice(VkInstance instance) -> std::pair<VkPhysicalDevice, std::uint32_t>
{
    ...
    std::multimap<int, std::pair<VkPhysicalDevice, std::uint32_t>> candidates{};
    std::ranges::transform(devices,
                           std::inserter(candidates, candidates.begin()),
                           [](const auto& device)
                           {
                               auto [rate, indices] = rateDeviceSuitability(device);
                               return std::make_pair(rate, std::make_pair(device, indices));
                           });
    ...
    return std::make_pair(candidates.rbegin()->second.first, candidates.rbegin()->second.second);
}

[[nodiscard]] auto createLogicalDevice(VkPhysicalDevice physicalDevice, std::uint32_t queueFamilyIndex)
    -> VkDevice
{
